...
列表本身是一个 Tab 落点：按 `Tab` 即可把焦点移到列表上，随后用方向键操作选中项。

| 按键 | 行为 |
| --- | --- |
| `Tab` / `Shift+Tab` | 焦点移入或移出列表 |
| `Up` / `Down` | 移动选中项，首次按下从第一行开始 |
| `Enter` | 确认当前选中行 |
| `Esc` | 取消选中 |

列表持有键盘焦点时，会在选中行上绘制焦点环；没有选中行时则画在列表本身上，让焦点始终显示在按键将要作用的那一行旁边。鼠标点击行同样会让列表获得焦点（方向键从点中的那一行继续），但鼠标操作不绘制焦点环。

可搜索列表则通过搜索框到达：`Tab` 落在搜索框上，输入和方向键都集中在一处，列表不再是第二个落点。
...
// 把焦点环交给外层控件绘制（需要引入 `FocusableExt`）
