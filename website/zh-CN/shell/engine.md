...
变的是这个数字*拿来跟什么比*。脚本的一次 `render` 不再等于一帧渲染：View 失效时会重建描述，[之后请求的帧可以复用它](./state.md#render-什么时候执行)——所以下面这笔脚本成本是按失效付，不是按重绘付。这让边界成本不像从前那么关键，但它并没有变成免费，而且仍然是决定要不要引入第二个引擎的那个数字。
...
| | 测的是什么 | 443 个节点 | 什么时候付 |
| --- | --- | --- | --- |
| **A** | 脚本 → Snapshot | **1.4 ms** | 基准中每次失效 |
| **B** | Snapshot → GPUI 元素 | **0.7 ms** | 基准中每次 materialize |
| **C** | 一整帧的缓存重绘 | **1.8 ms**，**没有执行脚本 `render`** | 基准中每次重绘 |

要用 release 跑，否则数字没有意义。本页所有绝对数值都来自一台 MacBook Pro（M3，8 核，24 GB）上的 release 构建，会随机器变化。

**C 既有断言，也有计时。** 对一个未变化的 View 重绘五十次，没有执行这个 View 的脚本 `render`；若执行了，基准会直接失败。这项测试没有测量空闲窗口的重绘频率，也不能排除其他界面中的帧路径回调。
...
描述一次，443 个节点是 1.1 ms；面板放大到 2,103、4,203、8,403 个节点，则依次是 5.1、10.3、20.5 ms。而一次实测的缓存重绘（包含 GPUI 布局与绘制）对应是 1.3、5.9、12.0、27.0 ms。两笔耗时都大致随节点数增长；本测试的缓存重绘不会重新执行该 View 的 JavaScript。对这个负载，可以得出三点：

- **4,203 个节点时，Snapshot 明显改变帧预算。** 实测 12 ms 的缓存重绘落在 60 Hz 的 16.67 ms 预算内；假如每帧再加上 10.3 ms 的描述成本，就会超过预算。这是针对该基准的预算比较，不是应用持续达到 60 FPS 的实测结论。
- **描述这笔成本没有消失，只是挪了位置。** 8,403 个节点的约 20 ms 在 View 失效时支付，而不是每次缓存重绘都支付；但它仍然是约 20 ms，所以单次调用成本依旧是评判第二个引擎的那个数字。
- **最大测试规模下，缓存帧不执行脚本 `render` 仍然很贵。** 实测 27 ms 的重绘包含 GPUI 元素、布局与绘制工作。如此大的 View 值得考虑虚拟化；单换更快的脚本引擎不能消除这笔实测帧成本。

把 A 对照设计给出的预算，答案仍是“在预算之内，但余量比预期少”：目标是一次脚本 `render` 花 1.5 ms，实测达标；但那份预算是按 800 个节点、每次记录操作约 150 ns 推出来的，而实测是 443 个节点上约 320 ns，三倍规模的面板放不进一次描述。变的是这件事有多要紧。假设旧模型在一秒内的 120 帧中每帧都重建这份 1.4 ms 的描述，仅描述就要花 168 ms；当前缓存帧路径只在失效时支付脚本成本。设计为超大面板列出的三条手段——压低单次调用成本、缓存未变化的子树、对长列表做虚拟化——依然[还没有实现](./elements.md#还没有的东西)，但它们现在是优化项，不再是前提条件。
...
合成基准把各项成本拆开测；Longbridge 行情终端则让它们在同一个真实负载中同时发生。下面的样本来自 release 构建，窗口处于活动状态，显示器分辨率为 3,840 × 2,160、刷新率为 144 Hz。Watchlist 持续接收实时行情，同时显示选中标的的详情与五日价格图。120 FPS 的目标对应每帧 **8.33 ms** 预算；屏幕刷新率与目标是两个不同数字，也都不保证实际达到的帧率。
