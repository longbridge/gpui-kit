...
请在选择组件或编写布局代码之前阅读本指南。它记录 GPUI Kit 在多年桌面应用开发中形成的产品判断：界面应当原生、克制、精确，并让用户无需猜测就能完成任务。
...
3. **先 token，后数值：** 颜色、圆角、字体与间距必须构成系统，避免无法响应主题的孤立字面量。GPUI 的样式 API 见 [Style](./style)。
4. **桌面约定优先于网页惯例：** 保留键盘操作、窗口框架、菜单、密集数据视图、可调整区域和持久导航。
5. **状态必须可见：** hover、焦点、选中、禁用、加载、校验与破坏性状态必须清楚且一致。
6. **几何表达包含关系：** 圆角、内缩与边界应构成一致的层级，叠放时也不能破坏。
7. **反馈必须真实：** 加载、提示与动效应解释实际状态或变化，不为即时操作增加仪式感。
...
GPUI Kit 通过 Rust 库以及 `gpui-base` / `gpui-component` 分层来实现这些原则。应用通常组合或适度封装公开组件；贡献者把真正可复用的行为放入 Base，把视觉决策留在上层。
...
圆角应像字号和控件尺寸一样，使用有限的主题档位（`sm`、`md`、`lg`、`xl` 及更大的表面档位），不要为每个元素发明新数值。大型容器可比内部控件使用更柔和的档位；紧凑控件与浮层则使用更小的档位。普通矩形的圆角应明显小于短边的一半。`radius_full()` 只用于有意设计的圆形与胶囊，提示框不应意外变成胶囊。

嵌套表面应呈现同心的曲线关系。用 `inner radius ≈ max(0, outer radius − inset)` 选择最接近且合适的主题档位，让转角处的边带保持均匀；重复出现的特殊组合若需要中间值，应定义具名的应用令牌。分段控件是一个完整轮廓：两端的分段顺着容器内侧圆角，中间分段保持直角，每处边界只保留一道等宽分隔线。

<img class="alignment-light" src="/radius-hierarchy.svg?v=20260923-easing" alt="浅色主题的聊天输入框：内缩表面、两侧按钮及提示文字的间距一起变化；红色参考圆显示转角共用圆心。">
<img class="alignment-dark" src="/radius-hierarchy-dark.svg?v=20260923-easing" alt="深色主题的聊天输入框：内缩表面、两侧按钮及提示文字的间距一起变化；红色参考圆显示转角共用圆心。">

圆角必须约束整个可见表面，而不只是描边。外角露出的部分应透出后方表面；子元素背景、选中填充与滚动内容也须遵循同一边界。在 GPUI 中，不要假定父级圆角会自动裁剪后代；应给内层设置相应圆角或适当裁剪，再用高对比背景检查明暗主题下的叠层。选择裁剪方式时仍须保留向外绘制的焦点环。
...
一个产品使用同一套图标。图标辅助 label，不能用猜谜替代陌生操作。仅图标按钮始终需要无障碍名称；是否添加视觉提示遵循下文的 tooltip 规则。填充或彩色图标用于表达状态，不用于让 toolbar 显得热闹。
...
标题栏首先是窗口框架。保留其拖动区域，不要把普通的标题单击绑定到低频编辑操作。重命名应由明确的对象命令触发，并在适当时提供键盘路径。
...
加载表现必须对应真实的等待。设置面板内容已经可用时，切换应立即完成；只有内容确实在加载且轮廓已知时，才使用骨架图。
...
- 只有符号广为人知时才使用图标按钮，并提供无障碍名称；含义或范围不清楚时再加工具提示。
...
Tooltip 用于补足信息，不用于重复显而易见的操作。陌生符号、模糊的作用范围或未在别处展示的有用快捷键才需要它。消息底部反复出现的复制、下载等命令应保留无障碍名称和键盘路径；悬停标签只会打断浏览。
...
动效应遵循交互的连续性。指针或滚动位置在相关目标间移动时，预览表面保持稳定，只更新内容或位置；交互结束后再关闭。每次更新都淡出、重现，会被感知为闪烁。
...
文档中的链接若指向有固定名称的 API、组件、源码文件或指南，链接文字沿用准确的英文名称，中文页面也一样。例如写 `Font`、`Render`、`Input element`、`Plot label`，不要翻译名称，也不要额外加上多余的 `GPUI Kit` 前缀。周围的说明仍用中文。链接只覆盖被引用的名称；“源码”等说明性文字放在链接之外。

跨语言共用的技术对比表保留能力项、框架名、API 名和模型名的既有英文写法；翻译表格周围的解释，不翻译这些技术标签。普通界面用词如有通行的中文名称，仍可在对比表之外使用中文。
...
- control 有无障碍名称；仅图标 control 仅在含义、范围或快捷键需要解释时使用 tooltip；
- 文字与有意义边界对比度充足；
- 状态不只依赖颜色；
