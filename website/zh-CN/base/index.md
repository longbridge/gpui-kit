...
[History](./history.md) 介绍两种用途明确不同的状态结构：`History` 是包含根条目、当前条目、后退与前进分支的导航轨迹，`UndoHistory` 则记录分组的 undo 与 redo 事务。[Plot](./plot.md) 是图表的无样式基础：比例尺、图形、坐标轴、把 `Plot` 变成子元素的 element，以及 hover 跟踪；颜色和动效时序都交给你的设计系统。
