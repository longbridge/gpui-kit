...
通过 `TextSelection::touch_selection` 读取快照，用 `TextSelection::observe_touch_selection` 在快照变化时重绘；`begin_edge_drag`、`update_edge_drag`、`end_edge_drag` 拖动其中一端，`select_all` 全选被按下的参与者，`close_edit_menu` 在菜单自身的命令执行后关闭菜单。绘制 handle 和菜单的一方需要在每帧 paint 时调用 `TextSelection::register_touch_ui` 登记各个表面的 bounds，这样落在这些表面上的按压不会清除它们所属的选区。参与者通过 `TextSelectionRegistration::with_selection_edges` 上报其选区两端的绘制位置，`TextView` 已为每个 inline 完成这一步；GPUI Component 的 `Root` 提供完整的展示实现。
